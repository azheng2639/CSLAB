...
class Iff(Formula):
    ...
    def variables(self):
        return self.first.variables().union(self.second.variables())
    def evaluate(self,values):
        #return Implies(self.first.evaluate(values),self.second.evaluate(values)) and Implies(self.second.evaluate(values),self.first.evaluate(values))
...
myformula = Iff(Implies(Variable('p'),Variable('q')),Or(Not(Variable('p')),Variable('q')))
...
def isTautology(formula):
    variableList = list(formula.variables())
    for vals in listAllPossibleValues(variableList):
        if (formula.evaluate(dict(vals)) == False):
            return False
    return True 
...
def isContradiction(formula):
    variableList = list(formula.variables())
    for vals in listAllPossibleValues(variableList):
        if (formula.evaluate(dict(vals)) == True):
            return False
    return True 
